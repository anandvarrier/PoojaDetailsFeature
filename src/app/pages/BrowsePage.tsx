...
import {
  Search, Star, Clock, MapPin, ChevronRight, ChevronLeft,
  Zap, X, SlidersHorizontal, Phone, MessageCircle,
  ChevronDown, ChevronUp, Filter, TrendingUp, Sparkles,
  ArrowRight, Check
} from "lucide-react";
import { Footer } from "../components/Footer";
import { ALL_POOJAS, FILTER_CONFIG, SEARCH_SUGGESTIONS, IMG, type Puja } from "../data/pujaData";
...
const SLIDES = [
  {
    id: 1,
    tag: "🌑 Amavasya · 11 May 2026",
    title: "Kaal Sarpa Dosh Puja",
    subtitle: "The most auspicious day to book this remedy. Next Amavasya slots are filling fast.",
    cta: "Book for This Amavasya",
    ctaSecondary: "View Details",
    bg: "#7C2D12",
    image: IMG.fireCeremony,
    badge: "12 slots remaining",
    badgeColor: "#ef4444",
  },
  {
    id: 2,
    tag: "🪐 Planetary Remedies",
    title: "Navagraha Shanti Puja",
    subtitle: "Balance all 9 planetary energies in a single comprehensive ritual at Ujjain.",
    cta: "Explore Packages",
    ctaSecondary: "Check Dates",
    bg: "#4338ca",
    image: IMG.astrology,
    badge: "Most Booked",
    badgeColor: "#C05621",
  },
  {
    ...
    tag: "⚡ Available 24/7",
    title: "Instant Pujas",
    subtitle: "Need a blessing right now? Our pandits are ready. Completed in under 60 minutes.",
    cta: "Book Instantly",
    ctaSecondary: "See All Instant Pujas",
    bg: "#047857",
    image: IMG.pujaThai,
    badge: "From ₹499",
    badgeColor: "#059669",
  ...
  {
    id: 4,
    tag: "🎉 Ganesh Chaturthi · Coming Soon",
    title: "Ganesh Puja & Atharvasheersha",
    subtitle: "Book early for Ganesh Chaturthi celebrations. Limited pandits at Siddhivinayak.",
    cta: "Pre-Book Now",
...
export function BrowsePage() {
  ...
  // Derived: filtered poojas
  const filteredPoojas = ALL_POOJAS.filter((p) => {
    if (instantOnly && !p.isInstant) return false;
    if (activeFilters.category?.length && !activeFilters.category.some((f) => p.category.includes(f) || p.benefits.includes(f))) return false;
  ...
  const popularPoojas = ALL_POOJAS.filter((p) => !p.isInstant).slice(0, 6);
  const instantPoojas = ALL_POOJAS.filter((p) => p.isInstant);

  return (
    <div style={{ backgroundColor: "#F8F7FA", fontFamily: "'Public Sans', sans-serif" }}>

      {/* ── PAGE TITLE ──────────────────────────────────────────────── */}
      <div className="px-4 sm:px-6 lg:px-8 pt-6 pb-3 max-w-7xl mx-auto">
        ...
        <p className="text-sm mt-1" style={{ color: "#6B7280" }}>
          Trusted pandits · 100% authentic rituals · Booking support 7 days a week
        </p>
      </div>
        ...
        >
          {SLIDES.map((s, i) => (
            ...
            >
              ...
              <div className="absolute inset-0" style={{ background: s.bg, opacity: 0.85 }} />
              <div className="absolute inset-0 flex flex-col justify-center px-6 sm:px-10 lg:px-14">
                <div className="max-w-xl">
                  ...
                  <p className="text-sm mb-5 leading-relaxed max-w-sm" style={{ color: "rgba(255,255,255,0.85)" }}>
                    {s.subtitle}
                  </p>
                  <div className="flex items-center gap-3 flex-wrap">
                    <button
                      onClick={() => navigate("/pooja/kaal-sarpa-dosh")}
                      className="px-5 py-2.5 rounded-full text-sm font-semibold text-white flex items-center gap-1.5 transition-all hover:opacity-90"
                      style={{ backgroundColor: "rgba(255,255,255,0.25)", border: "1.5px solid rgba(255,255,255,0.5)" }}
                    > {s.cta}<ArrowRight size={14} /></button>
                    <button
                      onClick={() => navigate("/pooja/kaal-sarpa-dosh")}
          ...
          <button
            ...
            className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center transition-all hover:opacity-100"
            style={{ backgroundColor: "rgba(0,0,0,0.4)", opacity: 0.7 }}
          >
            <ChevronLeft size={18} className="text-white" />
          </button>
          <button
            onClick={() => setSlide((slide + 1) % SLIDES.length)}
            className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center transition-all hover:opacity-100"
            style={{ backgroundColor: "rgba(0,0,0,0.4)", opacity: 0.7 }}
          >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <TrendingUp size={17} style={{ color: "#E77237" }} />
            <h2 style={{ fontFamily: "'Public Sans', sans-serif", color: "#1C1917", fontSize: "1.1rem", fontWeight: 700 }}>Popular Pujas</h2>
          </div>
          <button
        ...
        <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide -mx-1 px-1">
          {popularPoojas.map((p) => (
            <PopularCard key={p.id} pooja={p} onClick={() => navigate(`/pooja/${p.id}`)} />
          ))}
        </div>
      </section>

      {/* ── INSTANT POOJAS ──────────────────────────────────────────── */}
      <section className="mb-10 py-8" style={{ backgroundColor: "#047857" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <Zap size={17} className="text-green-300" />
              <h2 style={{ fontFamily: "'Public Sans', sans-serif", color: "white", fontSize: "1.1rem", fontWeight: 700 }}>
                Instant Pujas
              </h2>
            </div>
            <button
              onClick={() => navigate("/all-pujas")}
              className="text-sm text-green-300 flex items-center gap-1 hover:opacity-80 transition-opacity"
            >
              View all <ChevronRight size={14} />
            </button>
          </div>
          <p className="text-sm mb-5" style={{ color: "rgba(255,255,255,0.7)" }}>
            ⚡ Available right now · Completed in under 60 minutes · From ₹499
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {instantPoojas.map((p) => (
              <div
                ...
                className="rounded-2xl overflow-hidden cursor-pointer transition-all hover:scale-105 hover:shadow-xl"
                style={{ backgroundColor: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)" }}
              >
                <div className="relative h-28 overflow-hidden">
                  <img src={p.image} alt={p.name} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/40" />
                  <span className="absolute top-2 left-2 text-xs px-2 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: "#059669" }}>
                    ⚡ Instant
                  </span>
                </div>
                <div className="p-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs" style={{ color: "rgba(255,255,255,0.6)" }}>{p.duration}</p>
                      <p className="font-bold text-green-300" style={{ fontFamily: "'Public Sans', sans-serif" }}>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            <Sparkles size={17} style={{ color: "#E77237" }} />
            <h2 style={{ fontFamily: "'Public Sans', sans-serif", color: "#1C1917", fontSize: "1.1rem", fontWeight: 700 }}>
              All Pujas
            </h2>
          </div>
          <button
            onClick={() => setShowMobileFilters(true)}
            className="lg:hidden flex items-center gap-1.5 text-sm px-4 py-2 rounded-full border font-medium relative"
            style={{ borderColor: "#E77237", color: "#E77237", backgroundColor: totalActiveFilters > 0 ? "#FFF0E9" : "white" }}
          >
            <Filter size={14} />
            Filters
          ...
          <div className="flex-1 min-w-0">
            ...
            {totalActiveFilters > 0 && (
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {Object.entries(activeFilters).flatMap(([group, ids]) =>
                  ids.map((id) => {
                    const config = FILTER_CONFIG[group as keyof typeof FILTER_CONFIG];
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <select
                ...
                style={{ borderColor: "#FED7AA", color: "#374151" }}
              >
            ...
            {filteredPoojas.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                {filteredPoojas.map((p) => (
                  <PoojaCard key={p.id} pooja={p} onClick={() => navigate(`/pooja/${p.id}`)} />
                ))}
              </div>
            ) : (
...
function PopularCard({ pooja, onClick }: { pooja: Pooja; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="flex-shrink-0 bg-white rounded-2xl overflow-hidden border cursor-pointer transition-all hover:shadow-md group"
      style={{ width: "220px", borderColor: "#FEE2E2" }}
    >
      <div className="relative h-32 overflow-hidden">
        <img src={pooja.image} alt={pooja.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        <span
          className="absolute top-2 left-2 text-xs px-2 py-0.5 rounded-full font-medium text-white"
          style={{ backgroundColor: pooja.tagColor }}
        >
          {pooja.tag}
        </span>
      </div>
      <div className="p-3">
        <p className="text-sm font-semibold text-gray-800 leading-tight mb-1">{pooja.name}</p>
        <div className="flex items-center gap-1 mb-2">
          <Star size={11} fill="#F59E0B" stroke="#F59E0B" />
          <span className="text-xs text-gray-500">{pooja.rating} ({pooja.reviews})</span>
        </div>
        <p className="font-bold text-sm" style={{ color: "#E77237", fontFamily: "'Public Sans', sans-serif" }}>
...
function PoojaCard({ pooja, onClick }: { pooja: Pooja; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
    ...
    >
      ...
      {/* Body — flex column so CTA stays pinned */}
      <div className="p-4 flex flex-col flex-1">
        <h3 className="font-semibold text-gray-800 text-sm leading-snug mb-1.5">{pooja.name}</h3>
        ...
        <div className="mt-auto pt-3 space-y-2">
          <div className="flex items-center gap-1 text-xs text-gray-500">
            ...
            {pooja.location
              ...
              .join(" · ")}
            {pooja.location.length > 2 && <span className="text-gray-400">+{pooja.location.length - 2}</span>}
          </div>
          <div className="flex items-center gap-3 text-xs text-gray-500">
            <span className="flex items-center gap-1">
              <Star size={11} fill="#F59E0B" stroke="#F59E0B" />
              {pooja.rating} ({pooja.reviews})
            </span>
            <span className="flex items-center gap-1">
              <Clock size={11} /> {pooja.duration}
            </span>
          </div>
...
function FilterGroup({
  config, activeValues, expanded, onToggleExpand, onToggleOption, onSelectAll,
}: {
  config: { label: string; icon: string; options: readonly { id: string; label: string; icon?: string; desc?: string }[] };
  activeValues: string[];
  expanded: boolean;
  onToggleExpand: () => void;
...
function MobileFilterDrawer({
  activeFilters, instantOnly, onToggleInstant, onToggleOption, onClear, onClose, totalCount,
}: {
